...
class User_visit extends Main_Controller {
  ...
  public function delete_visits_for_user($user_id = NULL){
    if($user_id != NULL){
     $this->user_visits_m->delete_visits_for_user($user_id);
   } 
   redirect('user_visit');
 }

 public function delete_visit($visit_id = NULL){
  if($visit_id != NULL){
    $this->user_visits_m->delete_visit($visit_id);
  }
  redirect('user_visit');
}
